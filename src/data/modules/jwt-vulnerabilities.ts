...
export const jwtVulnerabilities: Module = {
    ...
    description: 'Understand JWT attacks including algorithm confusion, weak secrets, and token tampering.',
...
4. **Reject "none" algorithm** - Explicitly disable it
        ...
        },
        {
            ...
            content: '',
            quiz: {
                question: "In an algorithm confusion attack, how does an attacker exploit a server that supports both RS256 and HS256?",
                options: [
                ...
                correctAnswer: 1,
                explanation: "In algorithm confusion, the attacker switches from RS256 (asymmetric) to HS256 (symmetric). If the server uses its public key for HMAC verification (because it's the only 'key' it has for that token), the attacker can sign with that same public key and the signature will verify."
            }
        },
        {
            id: 'jwt-quiz-2',
            ...
            content: '',
            quiz: {
                question: "What is the correct way to prevent 'none' algorithm attacks?",
                options: [
                ...
                correctAnswer: 1,
                explanation: "The safest approach is to explicitly whitelist the algorithms your server accepts (e.g., only HS256 or only RS256) and reject any token that specifies a different algorithm, including 'none'. Never trust the algorithm specified in the token header."
            }
        },
        {
            id: 'jwt-quiz-3',
            ...
            content: '',
            quiz: {
                question: "For HMAC-SHA256 signed JWTs, what is the minimum recommended secret length?",
                options: [
                ...
                correctAnswer: 2,
                explanation: "HMAC-SHA256 produces a 256-bit hash, so your secret should have at least 256 bits of entropy to prevent brute-force attacks. This means at least 32 random bytes, not a human-readable password. Weak secrets can be cracked in minutes with tools like hashcat."
            }
        },
        {
            id: 'jwt-quiz-4',
            ...
            content: '',
            quiz: {
                question: "Besides verifying the signature, which JWT claims should ALWAYS be validated?",
                options: [
                ...
                correctAnswer: 2,
                explanation: "A valid signature only proves the token wasn't tampered with—it doesn't mean the token should be accepted. You must check: exp (not expired), iss (from trusted issuer), aud (intended for your service). Otherwise, tokens from other services or expired tokens could be accepted."
            }
        },
        {
            id: 'jwt-lab',
            ...
            type: 'lab',
            content: 'The JWT verification code below is vulnerable to algorithm confusion and accepts the "none" algorithm. Your mission: Implement secure verification with algorithm whitelisting.',
            lab: {
                initialCode: `
...
function verifyToken(token, publicKey) {
  ...
  return payload;
                ...
                `,
                instructions: "Fix the JWT verification: 1) Create an allowedAlgorithms array with only 'RS256', 2) Use this whitelist in jwt.verify options, 3) Add issuer and audience validation."
            }
        }
    ]
};
